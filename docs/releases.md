...
The current source builds the `0.9.2` portable **plugin-only** release. It contains the four
...
python scripts/release.py verify dist/embedded-agent-toolkit-0.9.2.zip --checksum dist/embedded-agent-toolkit-0.9.2.zip.sha256 --json
```
...
```powershell
python scripts/release.py install dist/embedded-agent-toolkit-0.9.2.zip `
  --checksum dist/embedded-agent-toolkit-0.9.2.zip.sha256 `
  --install-root C:\Tools\embedded-agent-toolkit `
  --lock-output C:\Tools\embedded-agent-toolkit\component-locks\workstation.json `
...
This creates `C:\Tools\embedded-agent-toolkit\0.9.2\embedded-agent-toolkit`,
