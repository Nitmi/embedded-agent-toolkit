...
The current source builds the `0.8.1` portable **plugin-only** release. It contains the four
...
python scripts/release.py verify dist/embedded-agent-toolkit-0.8.1.zip --checksum dist/embedded-agent-toolkit-0.8.1.zip.sha256 --json
```
...
```powershell
gh release download v0.8.1 --repo Nitmi/embedded-agent-toolkit `
...
```powershell
python scripts/release.py install dist/embedded-agent-toolkit-0.8.1.zip `
  --checksum dist/embedded-agent-toolkit-0.8.1.zip.sha256 `
...
This creates `C:\Tools\embedded-agent-toolkit\0.8.1\embedded-agent-toolkit`,
